...
#W_s2 = tf.get_variable("W_s2",(300,voca_size),tf.float32,init)
#bias2 = tf.get_variable("bias2",(voca_size),tf.float32,init)


# What if given 2 layers? 

#W_input = tf.get_variable("W_input",(1,200,200),tf.float32,init)
#bias_input = tf.get_variable("W_bias",(200),tf.float32,init)
W_1 = tf.get_variable("W_1",(300,100),tf.float32, init)
bias_1 = tf.get_variable("b_1",(100),tf.float32, init)

W_2 = tf.get_variable("W_2",(100,voca_size),tf.float32,init)
bias_2 = tf.get_variable("b_2",(voca_size),tf.float32,init)


#last_sr = tf.concat(1,[last_s,last_r]) 	# [Batch_size , 600]
cell_s = GRUCell(200,150,"s")
cell_q = GRUCell(200,150,"q")
cell_q_s = GRUCell(200,150,"q_s")
...
score_1     = tf.nn.relu(tf.matmul(predic_drop,W_1) + bias_1)

score_drop  = tf.nn.dropout(score_1,dropout_rate)
score 	    = (tf.matmul(score_drop,W_2) + bias_2)


loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(score,t))
reg1  = 0.01 * tf.nn.l2_loss(W_1)
reg2  = 0.01 * tf.nn.l2_loss(W_2)
cost = tf.reduce_mean(loss + reg1 + reg2 )
