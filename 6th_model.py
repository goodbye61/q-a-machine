...
W_s2 = tf.get_variable("W_s2",(195,voca_size),tf.float32,init)
...
#scaling_vec = tf.get_variable("scaling",(190,190),tf.float32,init)
scaling_vec = tf.get_variable("scaling",(1),tf.float32,init)
scaling_bias = tf.get_variable("scaling_bias",(195),tf.float32,init)

#last_sr = tf.concat(1,[last_s,last_r]) 	# [Batch_size , 600]
cell_s = GRUCell(200,195,"s")
cell_q = GRUCell(200,195,"q")
cell_q_s = GRUCell(200,195,"q_s")
...
predic_1 = last_q_s + ((last_q * scaling_vec) + scaling_bias)
...
reg1  = 0.005 * tf.nn.l2_loss(W_s2)
#reg2  = 0.03 * tf.nn.l2_loss(scaling_vec)
